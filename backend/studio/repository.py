...
class NodeRepository:
    ...
    async def get_node(self, node_id: int) -> Optional[dict]:
        ...
        return await self.database.fetch_one(query, {"node_id": node_id})
    ...
    async def delete_node(self, node_id: int) -> int:
        ...
        return await self.database.execute(query, {"node_id": node_id})
    ...
    async def get_child_nodes(self, node_id: int) -> List[dict]:
        ...
        """
        children = await self.database.fetch_all(query, {"node_id": node_id})
    ...
    async def get_parent_nodes(self, node_id: int) -> List[dict]:
        ...
        """
        parents = await self.database.fetch_all(query, {"node_id": node_id})
...
class EdgeRepository:
    ...
    async def get_edge(self, edge_id: int) -> Optional[dict]:
        ...
        return await self.database.fetch_one(query, {"edge_id": edge_id})
    ...
    async def delete_edge(self, edge_id: int) -> int:
        ...
        return await self.database.execute(query, {"edge_id": edge_id})
...
class ProjectRepository:
    ...
    async def create_project(self, name: str, description: Optional[str] = None,
        ...
        query = """
        INSERT INTO project (name, description, root_node_id) 
        VALUES (:name, :description, :root_node_id)
        """
        values = {"name": name, "description": description, "root_node_id": root_node_id}
        return await self.database.execute(query, values)

    async def get_project(self, project_id: int) -> Optional[dict]:
        query = "SELECT * FROM project WHERE id = :project_id"
        return await self.database.fetch_one(query, {"project_id": project_id})

    async def get_all_projects(self) -> List[dict]:
        query = "SELECT * FROM project"
        return await self.database.fetch_all(query)

    async def update_project(self, project_id: int, name: Optional[str] = None, description: Optional[str] = None,
                             root_node_id: Optional[int] = None) -> int:
        values = {"name": name, "description": description, "root_node_id": root_node_id, "project_id": project_id}
        query = """
        UPDATE project SET 
    ...
    async def delete_project(self, project_id: int) -> int:
        ...
        return await self.database.execute(query, {"project_id": project_id})
