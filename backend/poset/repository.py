...
class NodeModel:
    ...
    async def create(cls, node: Node, parent_id: Optional[int] = None) -> Node:
        async with database.get_db().transaction():
            ...
            node_id = await database.get_db().execute(query=query, values=values)
            node.id = node_id

            if parent_id:
                # Check if the parent node exists
                query = "SELECT * FROM nodes WHERE id = :id"
                parent_node = await database.get_db().fetch_one(query=query, values={"id": parent_id})
                if not parent_node:
                ...
                await database.get_db().execute(query=query, values=values)
    ...
    @classmethod
    async def get(cls, node_id: int) -> Node | None:
        query = "SELECT * FROM nodes WHERE id = :id"
        node_data = await database.get_db().fetch_one(query=query, values={"id": node_id})
    ...
    @classmethod
    async def update(cls, node_id: int, node: Node) -> Node:
        async with database.get_db().transaction():
    ...
    async def delete(node_id: int) -> None:
        async with database.get_db().transaction():
            ...
            await database.get_db().execute(query=query, values={"id": node_id})
    ...
    async def get_parents(node_id: int) -> List[Node]:
        ...
        """
        rows = await database.get_db().fetch_all(query=query, values={"node_id": node_id})
...
class EdgeModel:
    ...
    async def create(edge: Edge) -> Edge:
        ...
        edge_id = await database.get_db().execute(query=query, values=values)
        return Edge(**edge.dict(), id=edge_id)

    @staticmethod
    async def read(edge_id: int) -> Edge:
        query = "SELECT * FROM edges WHERE id = :id"
        edge = await database.get_db().fetch_one(query=query, values={"id": edge_id})
        if edge is None:
            return None
        return Edge(**edge)
    ...
    async def delete(edge_id: int) -> None:
        ...
        await database.get_db().execute(query=query, values={"id": edge_id})
